$(function(){
  ...
  /* header为全屏减time高度 */
  $('#header').height($(window).height() - $('#time').height());
...
/* 自动轮播 */
  $('.sec .show ul').each(function(index,ele){
    ...
    $(ele).width(num*$(ele).find('li').eq(0).width());
    var current = 0;
    $(ele).timer = setInterval(function(){
      ...
      $(ele).animate({'left':(0-current*$(ele).find('li').eq(0).width())},600,'swing');
    },5000);
  });
/* .show高度调整 */
...
/* 点击参与滚动到页面底部 */
  $('.jump').on('click',function(){ 
    $('#fix').show(5,function(){
      $('#fix').find('.con').css({'top':($('#fix').height()-$('#fix').find('.con').height())/2,'left':($('#fix').width()-$('#fix').find('.con').width())/2});
    });
